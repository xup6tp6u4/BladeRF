...
% 隨機生成符碼點的數組，確保其長度與 nCarriers 相同
symbolmap = randn(1, nCarriers) + 1i * randn(1, nCarriers);

% 生成 OFDM 波形
OFDM = zeros(nCarriers, length(t));
for n = 1:nCarriers
    if n <= length(symbolmap)  % 確保索引不會超出範圍
        OFDM(n, :) = symbolmap(n) .* exp(1i * 2 * pi * fspace * n * t); % 將符碼載入個不同的子載波
    else
        warning('symbolmap 中的符碼點數不足，無法為第 %d 個子載波生成 OFDM 波形。', n);
    end
end
...
figure(4);
% 設定顯示的 RF 波形範圍
start_carrier = 60;  % 起始子載波 
end_carrier = 65;    % 結束子載波

% 只顯示從 start_carrier 到 end_carrier 的 RF 波形
for n = start_carrier:end_carrier
    subplot(end_carrier - start_carrier + 1, 1, n - start_carrier + 1);  % 計算顯示的子圖數量
    plot(t, real(RF(n, :)));  % 只繪製 RF 的實部
    title(['第 ', num2str(n), ' 個子載波的 RF 波形']);
    xlabel('Time (s)');
    ylabel('Amplitude');
